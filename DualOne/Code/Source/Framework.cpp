...
void Framework::DebugShowInformation()
{
	...
	if ( ImGui::BeginIfAllowed() )
	{
		...
		if ( ImGui::TreeNode( u8"âÊñ ÉVÉFÉCÉNÉeÉXÉg" ) )
		{
			...
			ImGui::SliderFloat( u8"Ç¬ÇÊÇ≥", &power, 0.1f, 128.0f );
			ImGui::SliderFloat( u8"é„Ç‹ÇËóÕÅièuä‘ÇÃÇ›óLå¯Åj", &decel, 0.05f, 64.0f );
			ImGui::SliderFloat( u8"óhÇÍéûä‘Åiâië±ÇÃÇ›óLå¯Åj", &time, 0.1f, 10.0f );
			ImGui::SliderFloat( u8"óhÇÍÇÃä‘äu", &interval, 0.0001f, 1.5f );
			if ( ImGui::Button( u8"éÌóﬁÇêÿÇËë÷Ç¶ÇÈ" ) )
			{
				kind = ( kind == Donya::ScreenShake::MOMENT )
				? Donya::ScreenShake::PERMANENCE
				: Donya::ScreenShake::MOMENT;
			}
			ImGui::Text( u8"ç°ÇÃéÌóﬁ : %s", ( kind == Donya::ScreenShake::MOMENT ) ? u8"èuä‘" : u8"âië±" );

			if ( ImGui::Button( u8"óhÇÁÇ∑ÅFÇw" ) )
			{
